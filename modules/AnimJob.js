...
export class AnimJob
{
	...
	Tick(ts)
	{
		...
		{
			// longest simulation frame duration is 0.2 seconds
			let frame_dt = Math.min(0.2, dt * 0.001);
			this.tsLastDelta = dt;
			this.tsLast = ts;
...
Modules.Report("Animation Jobs", 'This module adds a reusable code component that handles operations that happen repeatedly, but only while the site is in the foreground.');
