...
export class PageTimekeep extends PageDescriptor
{
		...
		instance.CreatePanel(
			...
			e =>
			{
				instance.e_root_list = instance.CreatePanel(
					...
					e =>
					{
						addElement(e, 'div', '', style_panel_title, e => { e.innerText = "Allocations" });
						instance.e_allocations_root = instance.CreatePanel(e, true, false, 'display:flex; flex-direction:column; flex-grow:1.0; overflow-y:auto; padding:var(--gap-05); gap:var(--gap-025); justify-content:flex-start;');
					}
				);
			}
		);
	...
	RefreshContent(instance, delay = -1)
	{
		...
		instance.e_allocations_root.innerHTML = '';
		...
		records.forEach(
			(x, i, a) =>
			{
				...
				let e_item = CreatePagePanel(
					instance.e_allocations_root, false, false, 'display:flex; flex-direction:row; border-radius:var(--gap-05); flex-basis:fit-content; flex-grow:0.0; flex-shrink:0.0; padding:var(--gap-05);',
					_ =>
					{
						...
						let fillbar = FillBar.Create(
							_,
							`${x.allocation_max - sum_used} hours remaining`,
							sum_used / x.allocation_max,
							{
								label_alt: `${Math.round(sum_used / x.allocation_max * 100)}% used`,
								from_hue_deg: 35.0,
								to_hue_deg: 65.0,
